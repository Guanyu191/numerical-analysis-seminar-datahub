...
> **Definition:** **Finite-difference formula**. A finite-difference formula is a set of weights $a_{-p},\ldots,a_q$ such that, for a suitable class of functions $f$, we have
> $$
> f'(x)\approx \frac{1}{h}\sum_{k=-p}^{q} a_k f(x+kh).
> $$
> The weights are independent of both $f$ and $h$. If the approximation becomes exact in the limit $h\to 0$ for a suitable class of functions, the formula is called convergent.
...
此时每个网格点上的函数值会参与多个不同位置的导数近似. 下面的例子会展示这一点.

**#2 常见的一阶差分**

在一阶导数近似里，有三个特别常见的子类.

> **Definition:** **Forward, backward, and centered FD formulas**. In a finite-difference formula, if $p=0$ it is called a forward-difference formula; if $q=0$ it is called a backward-difference formula; if $p=q$ it is called a centered-difference formula.
...
> **Example:** **Reusing the same differences on a grid**. Let $f(x)=x^2$, and take $h=1/4$ on the interval $[0,1]$. The grid points are then $0,1/4,1/2,3/4,1$.
> $$
> f(0)=0,\quad f(1/4)=1/16,\quad f(1/2)=1/4,\quad f(3/4)=9/16,\quad f(1)=1.
> $$
> This gives four forward-difference approximations,
> $$
> \begin{aligned}
> f'(0) &\approx 4\,(1/16-0),\\
...
> \end{aligned}
> $$
> It also gives four backward-difference approximations,
> $$
> \begin{aligned}
...
> \end{aligned}
> $$
> The same four differences appear repeatedly; what changes is only which node we interpret them as approximating.

从上面的例子可以看出，前向差分与后向差分的主要区别，其实只是我们把

$$
\frac{f(x+h)-f(x)}{h}
$$

理解成 "在左端点估计导数" 还是 "在右端点估计导数". 从对称性出发，一个自然想法是：把它理解成 "在中点估计导数". 这就引出了中心差分公式.

下面推导最短的中心差分公式，取 $p=q=1$. 为了不影响结论，我们把目标点平移到 $x=0$，于是可用的函数值是 $f(-h),f(0),f(h)$. (上面的前向差分，本质上是穿过 $(0,f(0))$ 与 $(h,f(h))$ 的直线斜率.) 我们可以改为：先用这三个点做二次插值，再对插值多项式求导.
...
对称性之外，中心差分相比单边差分还有一个重要优势：在后面的 **5-5-有限差分的收敛性** 中会看到，中心差分更有利于获得更高的精度阶.

原则上，任何有限差分公式都可以用同一套 "插值-求导" 的思路推导出来. 下面两个表格给出了一些常用结果：一个是中心差分，另一个是前向差分. 两个表格都对应 "在 $x=0$ 处估计 $f'(0)$". 后向差分可以由前向差分得到：把系数顺序反过来，并改变符号.
...
> **Example:** **Three finite-difference formulas for $f'(0)$**. The table above gives the following three concrete finite-difference formulas for $f'(0)$:
> $$
...
> $$
>
> **Demo:** **Comparing FD formulas for $f'(0)$**. If $f(x)=e^{\sin x}$, then $f'(0)=1$.
>
> ```Python
> import numpy as np
>
...
> ```
>
> Centered differences are usually more accurate than one-sided differences when the same step size $h$ is used.

**#3 高阶导数的差分公式**

很多应用需要二阶导数. 一个诱人的想法是 "差分的差分". 例如，把前面的中心一阶差分公式应用到 $f'$ 上，然后再用中心差分去近似出现的 $f'$，可以得到
...
更好且更容易推广的做法，是回到上面的二次插值多项式 $Q(x)$，用 $Q''(0)$ 近似 $f''(0)$. 这给出最简单的中心二阶差分：
...
对二阶导数而言，把前向差分改成后向差分时，只需要把权重顺序反过来，而不需要改变符号.
...
> **Function:** **fdweights**. **Fornberg's algorithm for finite-difference weights**
>
> ```Python
> import numpy as np
>
...
> **Demo:** **Finite differences on arbitrary nodes**. We use five nodes to estimate the derivative of $f(x)=\cos(x^2)$ at $x=0.5$.
>
> ```Python
> import numpy as np
>
