...
> **Demo:** **Why uniform nodes can be inefficient**. We plot an integrand whose oscillations get stronger to the right and compare trapezoid errors on the two halves of the interval.
>
> ```python
> import numpy as np
...
> ```
>
> With the same number of nodes, the trapezoid formula is noticeably more accurate on the left half-interval than on the right one.
...
> **Note:** 这里把 $R_f(4n)-S_f(4n)$ 当作误差估计，是因为 $R_f(4n)$ 比 $S_f(4n)$ 多消掉了一个主误差项. 所以在渐近区间里，它们的差通常会和 $S_f(4n)$ 的主误差处在同一个量级.
...
因此通常同时使用绝对与相对误差容忍度，当满足其中之一时停止. 一个常见写法是

$$
|E| < \delta_a + \delta_r |S|,
$$

其中 $\delta_a$ 与 $\delta_r$ 分别是绝对与相对容忍度，$S$ 表示当前的 Simpson 近似值.

当误差估计未通过上述测试时，我们把区间 $[a, b]$ 二分，并把当前误差预算也平分给两个半区间. 再利用恒等式

$$
\int_a^b f(x)\,dx = \int_a^{(a + b)/2} f(x)\,dx + \int_{(a + b)/2}^b f(x)\,dx
$$

分别计算两个半区间上的积分近似. 每个半区间都会递归地应用 Simpson 公式与误差估计准则，并各自使用一半的容忍度；必要时继续二分. 在计算机科学里，这类策略通常叫 **divide and conquer (分治)**: 把问题递归地拆成更容易的子问题，然后把结果拼起来.

**#4 一个实现**

在实现自适应积分时，通常只用最基本的 Simpson 值 $S_f(4)$ 以及它的误差估计 $E$ 来做决策. 计算 $S_f(4)$ 需要三个梯形估计 $T_f(1), T_f(2), T_f(4)$. 对 $T_f(4)$ 来说，被积函数只需要在 5 个节点上计算函数值；而这 5 个函数值已经足够复用来计算 $T_f(1)$ 与 $T_f(2)$.

下面的函数展示了如何把误差估计与递归二分结合起来. 它除了返回积分近似值，还返回自适应过程中实际使用到的所有节点，便于可视化节点分布. 这里把参数 `tol` 理解为当前区间可用的局部误差预算.

> **Function:** **intadapt**. **Adaptive integration with error estimation**
>
> ```python
> import numpy as np
>
...
>         nodes = [a, xl, m, xr, b]
>     else:
>         QL, tL = intadapt(f, a, m, 0.5 * tol, fa=fa, fb=fm, m=xl, fm=fl)
>         QR, tR = intadapt(f, m, b, 0.5 * tol, fa=fm, fb=fb, m=xr, fm=fr)
>         Q = QL + QR
...
> ```
>
> The nodes naturally cluster where the integrand changes more rapidly.
>
...
> ```
>
> If we measure work by the number of nodes actually chosen, the observed convergence is often close to fourth order, although the error estimator is not perfectly reliable in every situation.

自适应与误差估计非常强大，但也有代价. 误差估计不可能在所有情况下都完美: 有时结果不如请求精度准确，有时函数值计算次数多于必要. 当积分是更大计算过程里的一个步骤时，还可能出现更微妙的问题.
