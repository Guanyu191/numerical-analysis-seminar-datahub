...
> **Definition:** **Truncation error of a finite-difference formula**. For a finite-difference method with weights $a_{-p}, \ldots, a_q$, its truncation error is defined by
> $$
...
> **Example:** **Truncation error of forward difference**. The truncation error of the forward-difference formula $(f(h)-f(0))/h$ is
> $$
> \begin{aligned}
> \tau_f(h)
> &= f'(0)-\frac{f(h)-f(0)}{h}\\
...
> \end{aligned}
> $$
> The key conclusion is that this truncation error is $O(h)$ as $h \to 0$.

在收敛方法里，我们关心 $\tau_f \to 0$ 的速度，也就是误差随 $h$ 变小的衰减速率.

> **Definition:** **Order of accuracy of a finite-difference formula**. If the truncation error of a finite-difference formula satisfies $\tau_f(h) = O(h^m)$ for a positive integer $m$, then $m$ is called the order of accuracy.

因此，上面的前向差分是 1 阶精度. 在其他条件相同的情况下，更高的精度阶更好，因为 $O(h^m)$ 随 $h \to 0$ 的消失速度更快. 一般来说，在有限差分公式中包含更多函数值 (也就是在 **5-4-有限差分** 的一般形式里增加权重个数) 会提高精度阶，这一点可以从那一节给出的表格中直接看到.

精度阶通常通过 Taylor 展开计算: 把 $\tau_f(h)$ 中出现的 $f(kh)$ 都在 $h = 0$ 附近展开，然后只保留最低阶的非零项.

> **Example:** **Truncation error of centered difference**. We now compute the truncation error of the centered-difference formula $(f(h)-f(-h))/(2h)$:
> $$
> \begin{aligned}
> \tau_f(h)
> &= f'(0)-\frac{f(h)-f(-h)}{2h}\\
> &= f'(0)-(2h)^{-1}\Bigl[(f(0)+hf'(0)+\tfrac{1}{2}h^2f''(0)+\tfrac{1}{6}h^3f'''(0)+O(h^5))\\
> &\qquad\qquad\ \ -(f(0)-hf'(0)+\tfrac{1}{2}h^2f''(0)-\tfrac{1}{6}h^3f'''(0)+O(h^5))\Bigr]\\
> &= -(2h)^{-1}\bigl[\tfrac{1}{3}h^3 f'''(0)+O(h^5)\bigr]\\
> &= -\tfrac{1}{6}h^2 f'''(0)+O(h^4).
> \end{aligned}
> $$
> Thus the method has order of accuracy $2$.
>
> **Demo:** **Observing convergence rates on a log-log plot**. We apply the forward and centered differences to $f(x) = \sin(e^{x + 1})$ at $x = 0$ and observe the convergence rates.
>
> ```python
> import numpy as np
...
> ```
>
> On a log-log plot, these error curves should be close to straight lines, and their slopes correspond to the orders of accuracy.
...
当截断误差阶数为 $m$ 时，结论会推广成下面这个结论.

> **Note:** 如果把总误差粗略写成 $C_1 h^m + C_2 \epsilon_{\rm mach} h^{-1}$，那么最优步长就出现在两项同量级的时候. 令 $C_1 h^m \approx C_2 \epsilon_{\rm mach} h^{-1}$，得到 $h^{m+1} \approx \epsilon_{\rm mach}$，于是 $h_{\rm opt} \approx \epsilon_{\rm mach}^{1/(m+1)}$.


> **Observation:** **Optimal node spacing with roundoff**. For a finite-difference method of accuracy order $m$ in the presence of roundoff, the optimal node spacing satisfies
> $$
> h_{\rm opt} \approx \epsilon_{\rm mach}^{\,1/(m+1)},
> $$
> and the optimal total error is about $\epsilon_{\rm mach}^{\,m/(m+1)}$.

换一种说法: 对一阶差分公式，我们最多只能期待大约 "半数" 的机器有效数字；随着 $m$ 增大，我们更接近用满机器精度. 更高阶的有限差分方法通常既更高效，也更不容易被舍入误差破坏.

> **Demo:** **FD error with roundoff**. We estimate $f'(0)$ for $f(x) = e^{-1.3x}$ with first-, second-, and fourth-order formulas and observe how roundoff affects the results.
>
> ```python
> import numpy as np
> import matplotlib.pyplot as plt
...
>     vals = f(nodes)
>     FD1.append(np.dot(np.array([0, 0, -1, 1, 0], dtype=float) / h, vals))
>     FD2.append(np.dot(np.array([0, -0.5, 0, 0.5, 0], dtype=float) / h, vals))
>     FD4.append(np.dot(np.array([1/12, -2/3, 0, 2/3, -1/12], dtype=float) / h, vals))
>
...
> ```
>
> As $h$ first decreases, the error usually falls because truncation error dominates. Once $h$ becomes extremely small, the error rises again because roundoff takes over.
