...
> **Definition:** **Truncation error of a finite-difference formula**. For the finite-difference method with weights $a_{-p},\ldots,a_q$, the truncation error is
> $$
...
> **Example:** **Truncation error of forward difference**. The forward difference formula given by $(f(h)-f(0))/h$ yields
> $$
> \begin{aligned}
> \tau_f(h)
> &= f'(0)-\frac{f(h)-f(0)}{h}\\
...
> **Definition:** **Order of accuracy of a finite-difference formula**. If the truncation error of a finite-difference formula satisfies $\tau_f(h)=O(h^m)$ for a positive integer $m$, then $m$ is the order of accuracy of the formula.
...
> **Example:** **Truncation error of centered difference**. We compute the truncation error of the centered difference formula $(f(h)-f(-h))/(2h)$:
> $$
> \begin{aligned}
> \tau_f(h)
> &= f'(0)-\frac{f(h)-f(-h)}{2h}\\
...
>
> **Demo:** **Observing convergence rates on a log-log plot**. We apply the forward and centered formulas to $f(x)=\sin(e^{x+1})$ at $x=0$.
>
> ```Python
> import numpy as np
...
> **Observation:** **Optimal node spacing with roundoff**. For computing with a finite-difference method of order $m$ in the presence of roundoff, the optimal spacing of nodes satisfies
> $$
...
> **Demo:** **FD error with roundoff**. We apply first-, second-, and fourth-order formulas to estimate $f'(0)$ for $f(x)=e^{-1.3x}$.
>
> ```Python
> import numpy as np
> import matplotlib.pyplot as plt
