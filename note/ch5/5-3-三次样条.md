...
> **Note:** 到这里为止，我们已经把端点插值统一写成了对同一个系数向量 $\mathbf{z}$ 的线性方程. 后面的 $\mathbf{H}, \mathbf{J}, \mathbf{E}$ 只是继续沿用这套写法，把一阶导连续和二阶导连续也压缩成矩阵形式.
...
> **Demo:** **Cubic spline interpolation**. We first plot a not-a-knot cubic spline interpolant.
>
> ```python
> import numpy as np
...
> ```
>
> **Demo:** **Convergence of cubic spline interpolation**. We then observe how the error decreases as the nodes are refined.
>
> ```python
> import numpy as np
> import matplotlib.pyplot as plt
