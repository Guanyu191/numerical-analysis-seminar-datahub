...
> **Definition:** **Cubic spline**. A cubic spline is a piecewise cubic function whose first and second derivatives are continuous on the whole interval.
...
> **Note:** 到这里为止，我们做的是三件事: 先把每段三次多项式写成 $a_k, b_k, c_k, d_k$；再把所有系数拼成一个长向量 $\mathbf{z}$；最后把 "端点插值" 改写成对 $\mathbf{z}$ 的线性方程. 后面出现的 $\mathbf{H}, \mathbf{J}, \mathbf{E}$，只是为了把 "一阶导连续"、"二阶导连续" 也压缩成同样的矩阵写法.
...
$$
b_k + 2 c_k h_k + 3 d_k h_k^2 = b_{k+1},\qquad k = 1, \ldots, n - 1.
$$
...
$$
2 c_k + 6 d_k h_k = 2 c_{k+1},\qquad k = 1, \ldots, n - 1.
$$
...
**Not-a-knot spline (not-a-knot 样条)**

$$
S_1'''(t_1) = S_2'''(t_1),\qquad S_{n-1}'''(t_{n-1}) = S_n'''(t_{n-1}).
$$

自然样条有一些重要的理论性质，但 not-a-knot 样条在逐点精度上通常更好，因此后面只考虑 not-a-knot 样条.
...
把这两条方程追加到前面的线性系统中，就得到规模为 $4n$ 的方阵线性系统. 这与分段线性插值很不同: 分段线性插值不需要解线性系统. 对三次样条而言，虽然也存在类似 "帽函数" 的一组基，但无法像前一节那样直接写出闭式的 cardinal 基函数，因此解线性系统是绕不开的.

**#4 一个实现**

> **Function:** **spinterp**. **Cubic not-a-knot spline interpolation**
>
> ```python
> import numpy as np
>
...
为了清晰起见，这份实现紧贴上面的推导: 把所有约束写成一个线性系统，组装并求解一次，然后用求得的系数反复计算样条的数值. 这样我们就不会在每次求值时重复做线性代数运算.

**#5 条件数与收敛性**

> **Demo:** **Cubic spline interpolation and convergence**. We first plot a not-a-knot cubic spline interpolant, then observe how the error decreases as the nodes are refined.
>
> ```python
> import numpy as np
...
> ```
>
> If fourth-order convergence is visible, the error curve should have slope close to $-4$ on the log-log plot.

除了比分段线性插值更光滑以外，not-a-knot 三次样条还把精度阶数提升到了 4. 更具体地说，在等距节点下有下面的误差界.

> **Theorem:** **Convergence of spline interpolation**. Let $f \in C^4[a, b]$, and let $S_n$ be the not-a-knot cubic spline interpolant of the data $(t_i, f(t_i))$, $i = 0, \ldots, n$, on the equally spaced nodes $t_i = a + ih$, where $h = (b - a) / n$.
> Then, for sufficiently small $h$, there exists a constant $C > 0$ such that
> $$
> \|f - S_n\|_\infty \le C h^4.
> $$

样条插值的条件数比前一节的分段线性情形复杂得多. 一方面，由于所有分段多项式的系数需要同时求解，数据 $\{y_i\}$ 中的每个值都会影响整个区间上的 $S$. 另一方面，$S$ 的取值幅度可能比所有的 $\{y_i\}$ 都更大. 更深入的讨论通常需要参考更高级的教材.
