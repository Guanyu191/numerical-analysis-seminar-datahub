...
> **Example:** A nonlinear elliptic PDE involving the Laplacian is
> $$
...
> **Function:** **elliptic**. **Newton-type solve for a nonlinear elliptic PDE (Chebyshev collocation)**
>
> ```Python
> import numpy as np
...
> **Demo:** **Deflection of a MEMS membrane**. We solve $\Delta u - \lambda/(u+1)^2 = 0$ with homogeneous Dirichlet boundary values.
>
> ```Python
> import numpy as np
>
...
> ```
>
> If we are confident that the PDE has been coded correctly, then the main remaining error source is the truncation error of the discretization. A direct check is to refine the grid slightly and observe how much the numerical solution changes.

**#5 Demo：稳态对流-扩散与 Allen-Cahn**

下面两个例子都是 "时间相关 PDE 的稳态极限"：把时间导数设为 0，就得到椭圆型方程.

> **Demo:** **Steady advection-diffusion**. We solve $1-u_x-2u_y+\varepsilon\Delta u=0$ on $[-1,1]^2$ with homogeneous Dirichlet boundary values.
>
> ```Python
> import numpy as np
...
> ```
>
> **Demo:** **Steady Allen-Cahn**. We solve $u(1-u^2)+\varepsilon\Delta u=0$ on $[0,1]^2$ with a nontrivial Dirichlet boundary condition.
>
> ```Python
> import numpy as np
> import matplotlib.pyplot as plt
