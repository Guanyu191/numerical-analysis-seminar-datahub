...
> **Definition:** **Tensor-product grid**. Given discretizations of two intervals,
> $$
...
>
> **Example:** Let $[0,2]$ be divided into $m=4$ equal pieces and $[1,3]$ be discretized into $n=2$ equal pieces. Then the grid points in $[0,2]\times[1,3]$ are $(i/2,\,1+j)$ for $i=0,1,2,3,4$ and $j=0,1,2$. If $f(x,y)=\sin(\pi xy)$, then $\operatorname{mtx}(f)$ is the matrix of all values $\sin(\pi x_i y_j)$.
>
...
> **Demo:** **A polar function on the unit disk**. We start from a function given in polar form, e.g. $f(r,\theta)=1-r^4$, and build a grid in $(r,\theta)$ space.
>
> ```Python
> import numpy as np
> import matplotlib.pyplot as plt
...
>
> **Demo:** **A function on the unit sphere (color as value)**. We plot $f(x,y,z)=xyz^3$ on the unit sphere, using color to represent the function values.
>
> ```Python
> import numpy as np
> import matplotlib.pyplot as plt
