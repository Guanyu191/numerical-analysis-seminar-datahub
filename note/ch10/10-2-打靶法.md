...
> **Demo:** **Trial initial values for the MEMS problem**. We solve the IVP repeatedly with different guesses of $w(0)$ and compare how close $w(1)$ gets to 1.
>
> ```Python
> import numpy as np
...
打靶法的实现结构很直接：外层用一个求根器；每次计算 $\mathbf{v}(\mathbf{s})$ 的数值时，内层都要解一次初值问题. 这也是为什么在求根阶段我们会关心 "目标函数调用次数"：每次调用都很贵.

> **Demo:** **A minimal shooting solver in Python**. We implement a generic `shoot` routine with `solve_ivp` (for the IVP) and `root` (for the rootfinding), and apply it to the MEMS problem.
>
> ```Python
> import numpy as np
> import matplotlib.pyplot as plt
...
>
> **Demo:** **Shooting instability as $\lambda$ increases**. We compute shooting solutions for several values of $\lambda$ and check whether the right boundary condition is satisfied.
>
> ```Python
> import numpy as np
> import matplotlib.pyplot as plt
