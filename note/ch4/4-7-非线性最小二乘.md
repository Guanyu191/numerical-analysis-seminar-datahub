...
> **Demo:** **Convergence of Gauss-Newton under different residual levels**. We perturb a function with a known zero-residual minimizer to create cases with different minimum residuals $R$, and observe the convergence behavior.
>
> ```Python
> import numpy as np
...
假设给定数据点 $(t_i,y_i)$，$i=1,\dots,m$. 我们用一个依赖参数 $\mathbf{x}\in\mathbb{R}^n$ 的模型函数 $g(t,\mathbf{x})$ 去拟合数据. 标准做法是定义 **misfit (拟合残差)** 向量
...
> **Demo:** **Michaelis-Menten data fitting**. We generate synthetic data from a Michaelis-Menten model and fit the parameters by nonlinear least squares.
>
> ```Python
> import numpy as np
> import matplotlib.pyplot as plt
