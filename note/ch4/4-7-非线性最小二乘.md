...
记 $\mathbf{y}_k=\mathbf{f}(\mathbf{x}_k)$. 如果把 $\mathbf{x}$ 写成 $\mathbf{x}_k+\mathbf{s}$，那么

$$
\mathbf{q}(\mathbf{x}_k+\mathbf{s})
=
\mathbf{y}_k+\mathbf{A}_k\mathbf{s}.
$$

在方阵情形 $m=n$ 时，我们通过解线性系统 $\mathbf{A}_k\mathbf{s}_k=-\mathbf{y}_k$ 得到步长 $\mathbf{s}_k=\mathbf{x}_{k+1}-\mathbf{x}_k$. 但在 $m>n$ 时，我们通常无法让 $\mathbf{q}(\mathbf{x}_k+\mathbf{s})=\boldsymbol{0}$ 精确成立，因此转而选择使线性模型残差最小的步长：
...
设最小化后得到的最小残差为 $\|\mathbf{f}(\mathbf{x}_*)\|_2=R>0$. 在不少问题中，会看到一种很常见的 "先快后慢"：

- 前期：迭代点离真最优解还远时，看起来有点像二次收敛.
- 后期：当误差进入 $\sim R$ 的尺度后，收敛往往会变成线性；如果 $R$ 不够小，后面可能会慢得很明显 (甚至出现停滞段).

特别地，如果最优点恰好满足 $R=0$，那么局部行为会更接近 Newton 方法，这时常常又能看到二次收敛.

> **Note：** 直觉上，当前误差还远大于 $R$ 时，问题看起来很像在解 $\mathbf{f}(\mathbf{x})=\mathbf{0}$；但当误差已经降到和 $R$ 同一个量级时，"最优点本身残差不为 0" 这一事实就开始主导，于是后期更容易显出线性收敛的尾部.

> **Demo:** **Convergence of Gauss-Newton under different residual levels**.
>
> **Note：** 这里先选一个已知零残差最优点，再加上大小为 $R$ 的常量扰动，让最小残差变成不同量级，然后比较误差曲线的变化.
> 
> ```Python
> import numpy as np
...
假设给定数据点 $(t_i,y_i)$，$i=1,\dots,m$. 我们用一个依赖参数 $\mathbf{x}\in\mathbb{R}^n$ 的模型函数 $g(t,\mathbf{x})$ 去拟合数据. 标准做法是把每个数据点上的预测误差收集成 **misfit (拟合残差)** 向量
...
如果我们想显式写出 Jacobian (雅可比矩阵)，那么矩阵元素为
...
当 $g$ 对参数是线性形式 (例如 $g(t,\mathbf{c})=\sum_j c_j g_j(t)$) 时，misfit (拟合残差) 也对 $\mathbf{c}$ 线性，从而退化回线性最小二乘.

> **Demo:** **Michaelis-Menten data fitting**.
>
> **Note：** 这里同时比较两种做法：一种直接在原始模型上做非线性最小二乘，另一种先做变量变换把问题线性化，再做线性最小二乘.
> 
> ```Python
> import numpy as np
> import matplotlib.pyplot as plt
