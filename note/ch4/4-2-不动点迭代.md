...
若 $L<1$，则 $g$ 会把距离整体缩小，因此称为 **contraction mapping**. 在这种情形下，我们至少能得到一个很强的结构性结论：在 $S$ 中不动点至多唯一；并且一旦不动点存在，不动点迭代的误差就有显式上界，从而得到收敛性保证. 在更标准的版本里，如果还满足 $g(S)\subseteq S$ 且 $S$ 是闭区间 (从而是完备的)，那么不动点的存在性也能得到保证 (Banach fixed-point theorem).
...
> $$
> **Proof (partial):** First we show there is at most one fixed point in $S$. If $p,q\in S$ satisfy $g(p)=p$ and $g(q)=q$, then
> $$
