...
> **Definition:** **Bandwidth**. A matrix $\mathbf{A}$ has upper bandwidth $b_u$ if $j-i>b_u$ implies $A_{ij}=0$, and lower bandwidth $b_\ell$ if $i-j>b_\ell$ implies $A_{ij}=0$. We say the total bandwidth is $b_u+b_\ell+1$. When $b_u=b_\ell=1$, we have the important case of a tridiagonal matrix.
>
...
> **Definition:** **Symmetric matrix**. A square matrix $\mathbf{A}$ satisfying $\mathbf{A}^{T} = \mathbf{A}$ is called symmetric.
...
> **Definition:** **Symmetric positive definite matrix**. A real $n\times n$ matrix $\mathbf{A}$ is called a symmetric positive definite matrix (or **SPD** matrix) if it is symmetric and, for all nonzero $\mathbf{x}\in\mathbb{R}^n$,
> $$
