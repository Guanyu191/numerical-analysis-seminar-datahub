...
> 这对应代码里的 `U[k, :] = Ak[k, :]`.
>
> 1. 取当前矩阵 $\mathbf{A}_k$ 的第 $k$ 列除以主元 $U_{kk}$，作为 $\mathbf{L}$ 的第 $k$ 列：
>
> $$
...
>
> 1. 用一个外积把第 $k$ 行与第 $k$ 列同时从剩余矩阵里消去，得到下一步的矩阵：
>
> $$
...
>
> **Definition:** **LU factorization**. Given $n\times n$ matrix $\mathbf{A}$, its LU factorization is
> $$
