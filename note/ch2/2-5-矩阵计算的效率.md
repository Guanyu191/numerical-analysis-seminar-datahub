...
> **Definition:** **Asymptotic notation**. Let $f(n)$ and $g(n)$ be positive-valued functions. We say $f(n) = O(g(n))$ (read "f is big-O of g") as $n \to \infty$ if $f(n)/g(n)$ is bounded above as $n \to \infty$. We say $f(n) \sim g(n)$ (read "f is asymptotic to g") as $n \to \infty$ if $f(n)/g(n) \to 1$ as $n \to \infty$.
