...
> **Definition:** **Matrix condition number**. The matrix condition number of an invertible square matrix $\mathbf{A}$ is
> $$
...
> **Definition:** **Residual of a linear system**. For the problem $\mathbf{A}\mathbf{x} = \mathbf{b}$, the residual at a solution estimate $\tilde{\mathbf{x}}$ is
> $$
