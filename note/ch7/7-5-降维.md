...
> **Demo:** **Low-rank SVD approximation for compression**. We compute the SVD of a grayscale image matrix, plot the singular values, and visualize low-rank reconstructions.
>
> ```Python
> import numpy as np
...
> **Demo:** **A low-dimensional structure hidden in data**. We use a synthetic "voting record" matrix to illustrate that a data set can be primarily low-dimensional, and that the first two singular directions can capture major trends.
>
> ```Python
> import numpy as np
> import matplotlib.pyplot as plt
