...
> **Definition:** **Backward error**. Let $\tilde f$ be an algorithm for the problem $f$. Let $y=f(x)$ be an exact result and $\tilde y=\tilde f(x)$ be its approximation by the algorithm. If there is a value $\tilde x$ such that $f(\tilde x)=\tilde y$, then the relative backward error in $\tilde y$ is
> $$
> \frac{|\tilde x-x|}{|x|}.
> $$
