...
这个现象称为 **subtractive cancellation**，也叫 **loss of significance**. 我们可以说，从 $-1.0012$ (5 位有效数字) 到 $-0.0012$ (2 位有效数字) 的映射中 "丢掉" 了 3 位有效数字. 一旦我们决定把所有量都舍入到固定的位数，这种有效位数的损失就无法通过更换算法来避免.

> **Observation:** Subtractive cancellation is a loss of accuracy that occurs when two numbers add or subtract to give a result that is much smaller in magnitude. It is one of the most common mechanisms introducing dramatic growth of errors in floating-point computation.
...
> **Definition:** **Condition number (scalar function)**. The relative **condition number** $\kappa_f(x)$ of a scalar function $f(x)$ is
> $$
...
> **Example:** Let's return to our "add 1" problem and generalize it slightly to $f(x)=x-c$ for constant $c$. We compute
> $$
...
> **Example:** Another elementary operation is to multiply by a constant: $f(x)=cx$ for nonzero $c$. We compute
> $$
...
> \begin{array}{ll}
> \hline
> \text{Function} & \text{Condition number}\\
> \hline
...
> **Example:** Consider the problem $f(x)=\cos(x)$. By the table above, $\kappa_f(x)=|x\tan(x)|$. There are two different ways in which $\kappa$ might become large:
...
> $$
> Based on the expression above, we can expect poor conditioning in the rootfinding problem if and only if $|r_1|\gg |r_1-r_2|$. Similar conclusions apply for $r_2$ and for variations with respect to the coefficients $b$ and $c$.

上面的计算可以推广到更高次多项式.

> **Observation:** Roots of polynomials are ill-conditioned with respect to changes in the polynomial coefficients when they are much closer to each other than to the origin.
