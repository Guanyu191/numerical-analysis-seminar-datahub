...
> **Example:** Consider the Black-Scholes PDE
> $$
...
>
> **Example:** Using a global Chebyshev differentiation matrix $\mathbf{D}_x$ instead, the boundary conditions become
> $$
...
> **Algorithm:** **Time derivative for a parabolic PDE (method of lines with boundaries)**. Given a value of $t$ and $\mathbf{v}$:
>
...
> **Function:** **parabolic**. **Solution of parabolic PDEs by the method of lines**
>
> ```Python
> import numpy as np
...
> **Demo:** **Heat equation on [-1,1] with Dirichlet boundary conditions**. We solve $u_t=u_{xx}$ on $[-1,1]$ with $u(-1,t)=0$ and $u(1,t)=2$.
>
> ```Python
...
> ```
>
> **Demo:** **Heat equation with a nonlinear source term**. We solve $u_t=u^2+u_{xx}$ on $[0,1]$ with homogeneous boundary conditions of different kinds.
>
> ```Python
...
>
> **Demo:** **Black-Scholes equation by the method of lines with boundary removal**. We solve
> $$
