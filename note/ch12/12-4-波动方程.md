...
> **Demo:** **Wave equation in Maxwell form (Dirichlet u at both ends)**. We solve $u_t=z_x$, $z_t=c^2u_x$ with $c=2$ and $u$ satisfying homogeneous Dirichlet boundary conditions.
>
> ```Python
> import numpy as np
...
> **Demo:** **Wave equation with discontinuous wave speed**. We take $c(x)=1$ for $x<0$ and $c(x)=2$ for $x>0$.
>
> ```Python
