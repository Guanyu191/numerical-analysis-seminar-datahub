...
> **Definition:** **Advection equation**. The advection equation in one dimension is
> $$
> u_t + c u_x = 0,
> $$
> where $c$ is constant.
...
> **Observation:** Solutions to the advection equation propagate with constant speed and fixed shape.
...
> **Demo:** **Advection by the method of lines (periodic)**. We solve $u_t + c u_x = 0$ on $[-4,4]$ with periodic end conditions by the method of lines.
>
> ```Python
> import numpy as np
...
> **Demo:** **Traffic flow by the method of lines (periodic)**. We use periodic boundary conditions on $[0,4]$ and solve the semidiscrete ODE with a stiff solver.
>
> ```Python
> import numpy as np
> import matplotlib.pyplot as plt
