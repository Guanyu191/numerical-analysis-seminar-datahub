...
> **Theorem:**
> If $\mathbf{x}$ satisfies $\mathbf{A}^{T}(\mathbf{A}\mathbf{x}-\mathbf{b})=\mathbf{0}$, then $\mathbf{x}$ solves the linear least-squares problem, i.e., minimizes $\|\mathbf{b}-\mathbf{A}\mathbf{x}\|_2$.
>
> **Proof:**
> Let $\mathbf{z}$ be any vector in $\mathbb{R}^{n}$. Write
> $$
...
> **Definition:** **Normal equations**. Given $\mathbf{A}\in\mathbb{R}^{m\times n}$ and $\mathbf{b}\in\mathbb{R}^{m}$, the normal equations for the linear least-squares problem are
> $$
...
> **Definition:** **Pseudoinverse**. Let $\mathbf{A}\in\mathbb{R}^{m\times n}$ with $m>n$. If $\mathbf{A}^{T}\mathbf{A}$ is nonsingular, then the pseudoinverse of $\mathbf{A}$ is the $n\times m$ matrix
> $$
...
>
> **Proof:**
> The first part is left to the exercises. For the second part, suppose that
...
> **Definition:** **Matrix condition number (rectangular case)**. Let $\mathbf{A}\in\mathbb{R}^{m\times n}$ with $m>n$. Define
> $$
