...
> **Definition:** **Lagrange cardinal polynomial**. Given distinct nodes $t_0,\dots,t_n$, the polynomial
> $$
> \ell_k(x)
> =
...
>
> **Demo:** **Lagrange cardinal function**. We construct $\ell_k$ from the product definition and confirm the cardinal property by plotting it and marking the node values.
>
> ```Python
> import numpy as np
...
>
> **Example:** **Interpolating samples of $\tan(x)$**. We construct the Lagrange interpolating polynomials of degrees $n=1$ and $n=2$ for $f(x)=\tan(x)$.
>
...
> **Definition:** **Error indicator function**. For a set of distinct nodes $t_0,\dots,t_n$, define
> $$
...
> **Demo:** **Interpolation error and an upper bound**. We interpolate $f(x)=\\log(x)$ on $[1,3]$ and compare the observed error to an upper bound based on $\\Phi$.
>
> ```Python
> import numpy as np
> import matplotlib.pyplot as plt
