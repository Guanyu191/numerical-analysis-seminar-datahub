...
> **Example:** **Truncation can be extremely inefficient**. Consider $f(x)=1/(1+x^2)$. Then
> $$
...
> **Demo:** **A transformed integrand with double exponential decay**. We compare the original and transformed integrands for $f(x)=1/(1+x^2)$, illustrating why truncating in `t` can be much more effective.
>
> ```Python
> import numpy as np
...
> **Demo:** **Double exponential integration on $(-\\infty,\\infty)$**. We implement the method by wrapping an adaptive integrator on a truncated `t`-interval.
>
> ```Python
> import numpy as np
...
> **Demo:** **Double exponential integration for endpoint singularities**. We implement the transformed integral on a truncated interval $(0,M)$ and apply adaptive integration.
>
> ```Python
> import numpy as np
> from math import isfinite
...
> **Demo:** **A simple singular integral**. We compute $\\int_{0}^{0.01} \\frac{1}{\\sqrt{x}}\\,dx$. First rewrite it to fit the $(0,1)$ setup by using $s=100t$:
> $$
