...
Lagrange 公式在理论上很有用，但并不适合做数值计算. 对每一个新的点 $x$，我们都需要把所有 cardinal 基函数 $\ell_k(x)$ 都算出来；每个 $\ell_k$ 都需要做 $n$ 项乘积，因此每个 $x$ 的总工作量是 $O(n^2)$. 更糟的是，Lagrange 公式在数值上是不稳定的.
...
> **Example:** **Barycentric form for the quadratic interpolant**. Consider the quadratic case $(n=2)$ from the previous section. The weights are
> $$
...
计算全部 $n+1$ 个权重需要 $O(n^2)$ 次运算. 但权重只依赖于节点，不依赖于数据. 一旦 $w_k$ 已经算好，对任意给定的 $x$，计算 $p(x)$ 的数值只需要 $O(n)$ 次运算.

> **Algorithm:** **polyinterp (barycentric)**. Given nodes $t_0,\dots,t_n$ and data $y_0,\dots,y_n$, precompute weights $w_0,\dots,w_n$ and evaluate
> $$
> p(x)=
> \frac{\sum_{k=0}^{n}\dfrac{w_k y_k}{x-t_k}}{\sum_{k=0}^{n}\dfrac{w_k}{x-t_k}},
> $$
...
>
> **Demo:** **Polynomial interpolation by the barycentric formula**. We implement a callable interpolant and apply it to values of $f(x)=\\sin(e^{2x})$ on $[0,1]$ using equispaced nodes.
>
> ```Python
> import numpy as np
