...
- 显式法需要维护历史 $f$ 值：每步只做一次新的函数值计算，其余来自缓存.
...
> **Function:** **ab4**. **4th-order Adams-Bashforth for an IVP**
>
> ```Python
> import numpy as np
>
...
> ```
>
> **Demo:** **Convergence of AB4**. We study AB4 on $u' = \\sin((u+t)^2)$ over $0\\le t\\le 4$ with $u(0)=-1$, using a high-accuracy reference from `solve_ivp`.
>
> ```Python
> import numpy as np
...
> **Function:** **am2**. **2nd-order Adams-Moulton (trapezoid) for an IVP**
>
> ```Python
> import numpy as np
...
> **Demo:** **A stiff problem where implicit wins**. We solve $u'=u^2-u^3$ on $[0,400]$ with $u(0)=0.005$. With the same number of steps, AB4 can go catastrophically wrong while AM2 remains reasonable.
>
> ```Python
> import numpy as np
> import matplotlib.pyplot as plt
