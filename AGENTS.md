...
- **“是 $x$ 的函数”改成“关于 $x$ 的函数”：** 在中文数学表述里，像 Jacobian、导数、矩阵值函数这类对象，通常说“它是关于 $\mathbf{x}$ 的函数”更顺，不要写成“它是 $\mathbf{x}$ 的函数”.
...
- **“评估 $f$”优先改成“计算 $f$ 的数值”：** 在中文数值分析笔记里，描述 function evaluation 时，优先写成“计算 $\mathbf{f}$ 的数值”或“函数值计算”，不要直接照搬“评估 $\mathbf{f}$”这种翻译腔说法.
